...
def bloque(r):
  ...
  <tr><td style="padding:16px 0;border-bottom:1px solid #e6e2dd">
    <div style="font-size:15px;font-weight:600;line-height:1.4">
      <a href="{r['url']}" style="color:#1c1a17;text-decoration:none">{r['nombre']}</a>
    </div>
...
rojas = sum(1 for r in nuevas if (r.get("precio") or {}).get("nivel") == "roja")
vale = [r for r in nuevas if (r.get("precio") or {}).get("nivel") in ("verde", "sin_monto")]

secciones = ""
if nuevas:
    secciones += (f'<h2 style="font-size:16px;margin:26px 0 4px">Nuevas oportunidades '
                  f'({len(nuevas)})</h2><table width="100%" cellpadding="0" '
                  f'cellspacing="0">{"".join(bloque(r) for r in nuevas)}</table>')
if cerrando:
    secciones += (f'<h2 style="font-size:16px;margin:26px 0 4px">Cierran pronto '
                  f'({len(cerrando)})</h2><table width="100%" cellpadding="0" '
                  f'cellspacing="0">{"".join(bloque(r) for r in cerrando)}</table>')
...
<p style="font-size:13.5px;color:#6b6560;margin:0 0 18px">
 {len(nuevas)} oportunidades nuevas de desarrollo de software, de
 {d['resumen']['activas_revisadas']:,} licitaciones activas revisadas.
 {rojas} con alerta de presupuesto. {len(vale)} valen la pena mirar.
</p>
{secciones}
...
asunto = f"Radar MP {d['fecha']}: {len(nuevas)} oportunidades"
if cerrando:
